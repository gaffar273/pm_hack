...
async function generateExport() {
  ...
  if (briefing.next_steps && briefing.next_steps.length > 0) {
    ...
    for (const step of briefing.next_steps) {
      ...
      // Schema uses `reason`; fall back to `reasoning` for backwards compat
      const stepReason = step.reason ?? step.reasoning;
      md += `- **${statusIcon} ${step.action}**\n`;
      if (stepReason) md += `  - *${stepReason}*\n`;
    }
    md += `\n---\n\n`;
  }
  ...
  md += `\n---\n\n*Generated by ContextMD in ${elapsed}s*\n`;
  const outPath = path.join(process.cwd(), 'briefing_report.md');
  fs.writeFileSync(outPath, md, 'utf8');
  console.log(`\n✅ Beautiful Export saved to: ${outPath} (${elapsed}s)`);
}
